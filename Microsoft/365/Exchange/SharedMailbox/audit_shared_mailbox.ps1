...
.SYNOPSIS
    ...
    Audita usuários internos Cloud-Only no Microsoft Entra ID, consulta propriedades de identidade e último login,

    identifica caixas de correio compartilhadas no Exchange Online e exporta os resultados para CSV..



.DESCRIPTION

    O script conecta ao Microsoft Graph e ao Exchange Online para consultar dados de identidade e destinatários.

    Considera usuários internos (UserType = Member) que não possuem sincronização do Active Directory local, utilizando onPremisesSyncEnabled para identificar contas Cloud-Only.

    Para cada usuário, coleta nome, UPN, departamento, cargo e último login registrado.

    Em seguida, verifica no Exchange Online se o UPN corresponde a uma caixa de correio compartilhada.

    Finally, it compiles this information and exports it to a CSV file named 'Entra_Cloud_User_Audit.csv' in the script's execution directory.
...
.NOTES
    ...
    - Microsoft Graph: User.Read.All e AuditLog.Read.All para leitura das propriedades de usuário e sign-in activity.

    - Exchange Online: permissão compatível com Get-EXORecipient, como View-Only Recipients ou superior.
...
.EXAMPLE
    ...
    Runs the script, prompts for authentication, performs the audit, and creates 'Entra_Cloud_User_Audit.csv'.
...
param (
    ...
    [string]$OutputPath = ".\Entra_Cloud_User_Audit.csv"
...
Write-Host "Iniciando auditoria de usuários Cloud-Only no Microsoft Entra ID..." -ForegroundColor Cyan
...
Write-Host "Buscando usuários Cloud-Only no Microsoft Entra ID..." -ForegroundColor Yellow
...
try {
    ...
    # Consulta usuários internos (Member) e inclui a propriedade necessária para identificar contas Cloud-Only.

    # Inclui signInActivity para consultar o último login registrado.

    # A filtragem de onPremisesSyncEnabled é feita localmente para manter o critério explícito.

    # A disponibilidade de signInActivity depende dos requisitos de licenciamento e permissões do tenant.
    ...
    foreach ($user in $users) {
        ...
        # Considera Cloud-Only quando a conta não está marcada como sincronizada do ambiente local.

        if ($null -eq $user.onPremisesSyncEnabled -or $user.onPremisesSyncEnabled -eq $false) {

            

            # Verifica se o destinatário correspondente é uma caixa de correio compartilhada.
            ...
            # Obtém o último login registrado, quando essa informação estiver disponível.

            $lastSignIn = if ($user.SignInActivity -ne $null) { $user.SignInActivity.LastSignInDateTime } else { $null }



            # Consolida os dados do usuário para o relatório.
    ...
    # Encerra as conexões antes de finalizar em caso de erro.
