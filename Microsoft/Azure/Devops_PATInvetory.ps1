...
# AZURE DEVOPS - INVENTÁRIO COMPLETO DE PATs
  
# CSV: Organização, Dev, E-mail, PAT, escopo, criação, expiração, status, dias para expirar, expira em 30 dias, expira em 60 dias, escopo de alto risco, organizações alvo, authorizationId
...
# O token utilizado para essa API precisa possuir vso.tokenadministration e a identidade precisa ter as permissões administrativas necessárias na organização
  

$AccessToken = Read-Host "YOUR_PAT"
...
#
# Essa lista pode ser ajustada conforme a política da empresa.
#
# A comparação procura qualquer um desses termos dentro do
# conjunto de scopes do PAT.
#
